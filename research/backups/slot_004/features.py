...
class SMCFeatureFactory:
    ...
    def _calculate_base_metrics(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        atr = true_range.rolling_mean(window_size=14, min_periods=14)
        atr_std = true_range.rolling_std(window_size=14, min_periods=14)
        ...
        bb_mean = pl.col("close").rolling_mean(window_size=20, min_periods=1)
        bb_std = pl.col("close").rolling_std(window_size=20, min_periods=1).fill_null(0.0)
    ...
    def _identify_swings(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        rolling_high = high.rolling_max(window_size=window_size, min_periods=window_size)
        rolling_low = low.rolling_min(window_size=window_size, min_periods=window_size)
    ...
    def _calculate_structure(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        bos_buffer = pl.col("true_range").rolling_mean(window_size=5, min_periods=1).fill_null(0.0)
