...
from antigravity.gateway.ws_consumer import TickConsumer
...
# ---------------------------------------------------------------------------
# WebSocket Endpoint — Tick Ingestion
# ---------------------------------------------------------------------------
    ...
    try:
        while True:
            ...
            try:
                ...
                logger.warning(
                    "ws.queue_full",
                    dropped_symbol=tick.symbol,
                    queue_size=app_state.tick_queue.qsize(),
                )
...
# ---------------------------------------------------------------------------
# WebSocket Endpoint — Simulated Exchange Feed (for development)
# ---------------------------------------------------------------------------
...
async def get_status() -> SystemStatus:
    ...
    elapsed = (datetime.now(timezone.utc) - app_state.start_time).total_seconds()
    return SystemStatus(
        uptime_seconds=round(elapsed, 1),
        ticks_ingested=app_state.ticks_ingested,
        buffer_height=app_state.feature_factory.buffer_height if app_state.feature_factory else 0,
        ...
        rolling_sharpe=app_state.rolling_sharpe,
        overseer_state=app_state.overseer_state,
        overseer_events=app_state.overseer.event_log[-20:] if app_state.overseer else [],
        drift_detected=bool(app_state.drift_detected),
        shadow_fork_active=bool(app_state.shadow_fork_active),
        last_price=float(app_state.last_price),
        prices=app_state.prices,
        queue_depth=app_state.tick_queue.qsize(),
        active_features=(
            app_state.feature_factory.get_feature_names()
            if app_state.feature_factory else []
        ),
        regime_n_components=(
            app_state.regime_classifier.n_components
            if app_state.regime_classifier else 0
        ),
    )
