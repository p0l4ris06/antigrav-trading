...
from antigravity.core.execution import OmniGateway
...
class DaemonConfig:
    ...
    model_path: str = str(Path(os.getenv("AG_MODEL_DIR", "models")) / "ppo_antigrav_latest.zip")
    ...
    # Paths
    log_dir: str = str(Path(os.getenv("AG_CONFIG_DIR", ".")) / "logs")
    state_file: str = str(Path(os.getenv("AG_CONFIG_DIR", ".")) / ".daemon_state.json")
...
class DaemonState:
    ...
    def _load(self) -> dict:
        lock_path = Path(self.path.parent) / f"{self.path.name}.lock"
...
async def fetch_equity(
    ...
    if fallback is not None:
        log.warning("Using configured fallback equity: %.2f", fallback)
        return fallback
    log.error("CRITICAL: Cannot determine account equity. No fallback provided.")
    log.error("Set one of: --equity CLI arg, or check exchange API permissions")
    raise RuntimeError("Account equity unavailable — cannot proceed with trading")
...
async def inference_cycle(
    ...
    # Maintain a canonical feature list to prevent misalignment / shifting
    CANONICAL_FEATURES = [
    ...
    if not validated.get("done"):
        expected = model.observation_space.shape[0]
        log.info("Model observation space verified: expected=%d columns. Features available in factory=%d columns.", expected, len(CANONICAL_FEATURES))
        validated["done"] = True

    # Build observation state with padding/truncation matching CANONICAL_FEATURES
    raw_state_list = []
    for col in CANONICAL_FEATURES:
        if col in feature_df.columns:
            raw_state_list.append(feature_df[col][-1])
        else:
            raw_state_list.append(0.0)
    raw_state = np.array(raw_state_list, dtype=np.float32)
    ...
    # Check for NaN/Inf in state vector before cleaning
    if not np.isfinite(current_state).all():
        nan_mask = ~np.isfinite(current_state)
        nan_features = [CANONICAL_FEATURES[i] for i, is_nan in enumerate(nan_mask) if is_nan and i < len(CANONICAL_FEATURES)]
        log.error("Feature extraction failed: NaN/Inf detected in %s", nan_features)
        return

    # Clean NaN/inf for model safety
...
@click.command()
@click.option("--symbol", default="ETH/USDT", help="Symbol to trade")
@click.option("--timeframe", default="15m", help="Timeframe (e.g., 15m, 1h, 4h)")
@click.option("--exchange", default="binance", help="Exchange name")
@click.option("--dry-run", is_flag=True, help="Run in dry-run mode")
def live(symbol, timeframe, exchange, dry_run):
