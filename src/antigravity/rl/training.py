...
from antigravity.features.technical import ExtendedFeatureFactory
from antigravity.rl.agent import KellyConvexEnv, init_agent
...
def main():
    ...
    default_data_dir = os.getenv("AG_DATA_DIR", "data")
    default_data_path = os.path.join(default_data_dir, "BTC_USDT_15m.parquet")
    parser.add_argument("--data", type=str, nargs="+", default=[default_data_path], help="Path to data file(s) or directories")
    ...
    models_dir = os.getenv("AG_MODEL_DIR", "models")
    os.makedirs(models_dir, exist_ok=True)
    # Save model weights
    model.save(os.path.join(models_dir, "ppo_antigrav_latest")) 
    # Save the VecNormalize stats (under models/vec_normalize.pkl)
    train_vec_env.save(os.path.join(models_dir, "vec_normalize.pkl"))
