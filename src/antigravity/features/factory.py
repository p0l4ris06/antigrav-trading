...
class FeatureFactory:
    ...
    def compute_features(self, n_bars: int | None = None) -> pl.DataFrame | None:
        ...
        MIN_ATR_FLOOR = 0.001  # Set a meaningful minimum (0.1% of typical spread)
        atr_safe = pl.col("atr").clip(lower_bound=MIN_ATR_FLOOR)
        df = df.with_columns(
            [
                (pl.col("spread") / atr_safe).alias("spread_atr_norm"),
                (
    ...
    def get_latest_features(self) -> np.ndarray | None:
        ...
        # Fix: use canonical feature list to avoid shape mismatch / shifting on pruning
        CANONICAL_FEATURES = [
            "obi", "spread", "atr", "vol_20", "vol_50", 
            "vwap_60s", "vwap_300s", "volume_ratio", "log_returns",
            "spread_atr_norm", "microprice_deviation", "bull_bos", "bear_bos", "pad_1", "pad_2"
        ]

        row_list = []
        for c in CANONICAL_FEATURES:
            if c in df.columns and c not in self._dropped_features:
                row_list.append(df[c][-1])
            else:
                row_list.append(0.0)

        row = np.array(row_list, dtype=np.float32)
        if not np.isfinite(row).all():
            self.last_had_nan = True
            nan_mask = ~np.isfinite(row)
            nan_features = [CANONICAL_FEATURES[i] for i, is_nan in enumerate(nan_mask) if is_nan]
            logger.error("features.nan_inf_detected", features=nan_features)
        else:
            self.last_had_nan = False
