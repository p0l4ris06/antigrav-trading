...
from antigravity.telemetry.tracing import get_tracer, span
        ...
        if self._features is not None:
            ...
            with tracer.start_as_current_span("feature_compute") as fc_span:
                ...
                if obs is not None:
                    ...
                    if getattr(self._features, "last_had_nan", False):
                        self._app_state.drift_detected = True
            ...
            if obs is not None:
                ...
                if self._agent is not None:
                    with tracer.start_as_current_span("rl_predict") as rl_span:
                        try:
                            # Align observation dimension to expected model shape
                            expected_dim = self._agent.model.observation_space.shape[0]
                            target_feat_dim = expected_dim - len(regime_proba)
                            if len(obs) < target_feat_dim:
                                obs_aligned = np.pad(obs, (0, target_feat_dim - len(obs)), mode='constant')
                            elif len(obs) > target_feat_dim:
                                obs_aligned = obs[:target_feat_dim]
                            else:
                                obs_aligned = obs

                            # Concatenate features + regime probabilities
                            full_obs = np.concatenate([obs_aligned, regime_proba]).astype(np.float32)
