...
class OptimizerConfig:
    ...
    target_files: list[str] = field(default_factory=lambda: [
        "src/antigravity/features/base.py",
    ])
    ...
    backup_dir: str = field(default_factory=lambda: str(Path(os.getenv("AG_CONFIG_DIR", ".")) / ".autoresearch_backups"))
    backup_keep: int = 5

    # Experiment history
    history_dir: str = field(default_factory=lambda: str(Path(os.getenv("AG_CONFIG_DIR", ".")) / "experiments"))
    history_file: str = "history.jsonl"

    # Logging
    log_dir: str = field(default_factory=lambda: str(Path(os.getenv("AG_CONFIG_DIR", ".")) / "logs"))
...
def get_mutation(
    ...
    backup_features = Path(cfg.backup_dir) / "slot_000" / "base.py"
    canonical_cols = re.findall(r'\.alias\("(\w+)"\)', file_contents.get("src/antigravity/features/base.py", ""))
    if not canonical_cols:
        canonical_cols = re.findall(r'"(\w+)"', file_contents.get("src/antigravity/features/base.py", ""))
...
def validate_mutation_syntax(code: str) -> tuple[bool, str]:
    """Parse code and validate basic syntax/semantics."""
    try:
        tree = ast.parse(code)
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    ...
                    if base_module not in {"polars", "numpy", "scipy"}:
...
def apply_mutation(
    ...
    for path, code in zip(target_files, blocks):
        ...
        # Parse and validate syntax and imports
        ok, reason = validate_mutation_syntax(code)
        if not ok:
            log.error("MUTATION REJECTED for %s: %s", path, reason)
            return False
